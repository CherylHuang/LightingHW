...
void CShape::SetShader(GLuint uiShaderHandle)
{
	...
	m_uiLightInView = glGetUniformLocation(m_uiProgram, "LightInView");			//第一個光源
	...
	m_uiLightInView2 = glGetUniformLocation(m_uiProgram, "LightInView2");		//第二個光源
	...
	m_uiLightInView3 = glGetUniformLocation(m_uiProgram, "LightInView3");		//第三個光源
	...
	m_uiLightInView4 = glGetUniformLocation(m_uiProgram, "LightInView4");		//第四個光源
...
// 將資訊更新到 Shader 中
	...
	glUniform4fv(m_uiLightInView, 1, m_vLightInView);		//第一個光源
	glUniform4fv(m_uiAmbient, 1, m_AmbientProduct);
	glUniform4fv(m_uiDiffuse, 1, m_DiffuseProduct);
	glUniform4fv(m_uiSpecular, 1, m_SpecularProduct);
	...
	glUniform4fv(m_uiLightInView3, 1, m_vLightInView3);		//第三個光源
	glUniform4fv(m_uiAmbient3, 1, m_AmbientProduct3);
	glUniform4fv(m_uiDiffuse3, 1, m_DiffuseProduct3);
	glUniform4fv(m_uiSpecular3, 1, m_SpecularProduct3);

	glUniform4fv(m_uiLightInView4, 1, m_vLightInView4);		//第四個光源
	glUniform4fv(m_uiAmbient4, 1, m_AmbientProduct4);
	glUniform4fv(m_uiDiffuse4, 1, m_DiffuseProduct4);
	glUniform4fv(m_uiSpecular4, 1, m_SpecularProduct4);
#endif
...
{
	// Lighting With GPU
	if (LightNum > 0 && LightNum <= 4) {	//第一個光源，更新 m_Light1

		m_vLightInView = m_mxView * m_Light1.position;		// 將 Light 轉換到鏡頭座標
